...
public class productListDAOimpl implements productListDAO{
	...
	public ArrayList<ProductListDTO> select(String categoryId, int currentPage, int numberPerPage) throws SQLException {
		...
		String sql = "SELECT * "
				...
				+ "        SELECT ROWNUM no, t.* "
				+ "        FROM ( "
				+ "                SELECT p.ID, MAX(p.SHIPPINGOPTIONID), MAX(p.sellerstoreid), MAX(s.SELLERNAME), MAX(p.brandid), MAX(b.brandname), MAX(p.PDNAME), MAX(p.UPDATEDAY), COALESCE(MAX(o.optionPrice), 0) AS optionPrice, COALESCE(MAX(o.optionPrice)-((MAX(o.optionPrice)/100)*MAX(c.spclDscnRt)), 0) AS sprice, COALESCE(MAX(c.spclDscnRt), 0) AS discount   "
