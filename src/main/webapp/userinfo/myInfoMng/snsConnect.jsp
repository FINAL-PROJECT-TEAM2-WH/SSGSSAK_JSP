...
<div id="content" class="content_myssg">
    <h2 class="stit">SNS연결설정</h2>
    <div class="connect_section">
        <ul class="cmem_connect_btn">
            <li class="notranslate">
                <label for="chk_slct_naver" class="btn_naver"><span class="tx_ko">네이버 로그인 연결</span><span class="tx_en">Connection with Naver</span><span class="tx_zh">NAVER登录连接</span></label><span
                    class="connect_switch"><input id="chk_slct_naver" type="checkbox" name="chk_slct" checked="" data-code="60" data-type="naver" data-name="네이버" data-connected=></span>
            </li>
            <li class="notranslate">
                <label for="chk_slct_kakao" class="btn_kakao"><span class="tx_ko">카카오 로그인 연결</span><span class="tx_en">Connection with Kakao</span><span class="tx_zh">Kakao登录连接</span></label><span
                    class="connect_switch"><input id="chk_slct_kakao" type="checkbox" name="chk_slct" data-code="80" data-type="kakao" data-name="카카오" data-connected=></span>
            </li>
            <li class="notranslate">
                <label for="chk_slct_apple" class="btn_apple"><span class="tx_ko">애플 로그인 연결</span><span class="tx_en">Connection with Apple</span><span class="tx_zh">Apple登录连接</span></label><span
                    class="connect_switch"><input id="chk_slct_apple" type="checkbox" name="chk_slct" data-code="70" data-type="apple" data-name="애플" data-connected=></span>
            </li>
            
                <li class="notranslate">
                    <label for="chk_slct_toss" class="btn_toss"><span class="tx_ko">토스 로그인 연결</span></label>
                    <span class="connect_switch"><input id="chk_slct_toss" type="checkbox" name="chk_slct" data-code="90" data-type="toss" data-name="토스" data-connected=></span>
                </li>
            

        </ul>
        <ul class="cmem_connect_notice">
            <li>자주 사용하는 SNS 계정과 연결 설정을 하여 간편하게 로그인하실 수 있습니다.</li>
...
<script type="text/javascript" src="https://sui.ssgcdn.com/ui/member/js/login/connect.js?v=20240508"></script>
...
<script type="text/javascript" defer="defer">
    ...
    var dom;
    $(".cmem_connect_btn input").click(function () {
        dom = $(this);
        if (dom.data('connected')) {    
            if ("10" == "40") {
                if (confirm("사업자회원은 " + dom.data('name') + " 로그인 연결 해지 후 재 연결이 불가능합니다.\n" + dom.data('name') + ' 로그인 연결을 해지하시겠습니까?')) {
                    snsCallback();
                }
                return;
            }
            if ("" == "true") {
                if (confirm("SNS 로그인 연결 해제를 하시면, 비밀번호 등록 또는 다른 SNS 로그인 연결을 하신 후 SSG.COM 로그인을 하실 수 있습니다.\n" + dom.data('name') + ' 로그인 연결을 해제 하시겠습니까?')) {
                    snsCallback();
                }
                return;
            }
            if (confirm(dom.data('name') + " 로그인 연결을 해제 하시겠습니까?")) {
                snsCallback();
            }
        } else {
            if ("10" == "40") {
                alert("사업자회원은 " + dom.data('name') + " 로그인 연결 서비스를 제공하지 않습니다. 통합회원 또는 간편회원 로그인 후 SNS로그인 서비스를 이용해주세요.");
                return;
            }
            if (dom.data('type') === "naver") {
                snsLogin.naver('connect');
            } else if (dom.data('type') === "kakao") {
                snsLogin.kakao('connect');
            } else if (dom.data('type') === "apple") {
                snsLogin.apple('connect');
            } else if (dom.data('type') === "toss") {
                snsLogin.toss('connect');
            }
        }
    })
    ;
    ...
    function snsCallback() {
        if (!dom) {
            alert('SNS 연동을 취소하셨거나 오류가 발생했습니다.');
            return;
        }
        var param = {
            snsTypeCd: dom.data('code')
            , useYn: dom.data('connected') ? "N" : "Y"
        }
        $.ajax({
            url: "/myssg/myinfoMng/snsLinkUpdate.ssg"
            , method: "POST"
            , data: param
            , beforeSend: function () {
            ...
            , error: function (jqXHR, textStatus, errorThrown) {
                if (param.useYn === "Y") {
                    alert('SNS 연동 중 오류가 발생했습니다.');
                } else {
                    alert('SNS 연동 해제 중 오류가 발생했습니다.');
                }
                loadingIndicator.hide();
                document.location.reload();
            }
